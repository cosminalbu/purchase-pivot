...
export const generatePurchaseOrderPDF = async (
  ...
  // Date box (now below PO number, with both dates on same line)
  yPosition += 15
  doc.rect(rightColumnX, yPosition, 80, 12)
  doc.setFontSize(8)
  doc.setTextColor(100, 100, 100)
  doc.text('ORDER DATE', rightColumnX + 2, yPosition + 4)
  doc.text('REQUIRED DATE', rightColumnX + 42, yPosition + 4)
  doc.setFontSize(9)
  doc.setTextColor(33, 33, 33)
  ...
  doc.text(purchaseOrder.delivery_date ? new Date(purchaseOrder.delivery_date).toLocaleDateString('en-AU') : 'TBD', rightColumnX + 42, yPosition + 9)
  ...
  if (includeLineItems && lineItems.length > 0) {
    ...
    autoTable(doc, {
      ...
      theme: 'grid',
      headStyles: {
        fillColor: [200, 200, 200], // Medium gray background for better distinction
    ...
    const totalsTableX = 150 // Align right edge of totals table with right edge of items table (20 + 180 - 50 = 150)
